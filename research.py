...
from typing import Dict, List, Optional, Tuple, Union           # Type hints for function signatures
...
BINANCE_VISION_TRADES_URL = "https://data.binance.vision/data/spot/daily/trades"
BINANCE_TRADE_CSV_COLUMNS = ["id", "price", "qty", "quote_qty", "time", "is_buyer_maker", "is_best_match"]
...
def download_trades(sym: str, hist_data_window: int, data_path: Optional[str] = None) -> None:
    """
    Download historical daily trade data for a symbol from Binance's public
    data archive (data.binance.vision) and cache it locally as parquet files,
    one file per day, matching the naming convention expected by
    load_timeseries / load_timeseries_range: '{sym}-trades-YYYY-MM-DD.parquet'.

    Args:
        sym: Symbol (e.g., 'BTCUSDT')
        hist_data_window: Number of hours of history to download, ending on
            the most recent fully completed UTC day
        data_path: Directory to cache downloaded files (default: './cache')
    """
    ...
    no_days = max(1, math.ceil(hist_data_window / 24))
    end_date = datetime.utcnow().date() - timedelta(days=1)
    dates = [end_date - timedelta(days=i) for i in range(no_days)][::-1]

    for day in tqdm(dates, desc=f"Downloading {sym} trades", unit="day"):
        day_str = day.strftime('%Y-%m-%d')
        file_path = os.path.join(data_path, f"{sym}-trades-{day_str}.parquet")

        if os.path.exists(file_path):
            continue

        url = f"{BINANCE_VISION_TRADES_URL}/{sym}/{sym}-trades-{day_str}.zip"
        response = requests.get(url, timeout=60)

        if response.status_code != 200:
            tqdm.write(f"[WARNING] No data for {sym} on {day_str}: HTTP {response.status_code}")
            continue

        with zipfile.ZipFile(io.BytesIO(response.content)) as zf:
            csv_name = zf.namelist()[0]
            with zf.open(csv_name) as f:
                trades = pl.read_csv(f, has_header=False, new_columns=BINANCE_TRADE_CSV_COLUMNS)

        trades = trades.with_columns([
            pl.col("price").cast(pl.Float64),
            pl.from_epoch(pl.col("time"), time_unit="us").alias("datetime"),
        ])

        trades.write_parquet(file_path)
